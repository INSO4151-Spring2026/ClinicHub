...
# Moves up: flask_bridge -> clinic-hub -> ClinicHub, then down into 'backend'
...
if __name__ == "__main__":
    print("--- ClinicHub Backend Link Active ---")
    print(f"Database Location: {db_path}")
    print("\n--- DETECTED FLASK ROUTES ---")
    for rule in app.url_map.iter_rules():
        print(
            f"Path: {rule.rule} | Methods: {rule.methods} | Endpoint: {rule.endpoint}"
        )
    print("----------------------------\n")
    ...
    # Port 5002 is what your Node.js proxy is looking for
