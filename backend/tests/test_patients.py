...
class TestGetPatient:
    def test_get_existing_patient(self, client, auth_headers, sample_patient):
        res = client.get(
            f"/api/patients/{sample_patient.patient_id}", headers=auth_headers
        )
...
class TestUpdatePatient:
    ...
    def test_update_nonexistent_patient(self, client, auth_headers):
        res = client.put(
            "/api/patients/99999", json={"first_name": "Ghost"}, headers=auth_headers
        )
        assert res.status_code == 404
...
class TestDeletePatient:
    ...
    def test_deleted_patient_no_longer_retrievable(
...
class TestListPatients:
    ...
    def test_list_search_by_email(self, client, auth_headers):
        self._create_patients(client, auth_headers, 3)
        res = client.get(
            "/api/patients?search=patient_list_1@test.com", headers=auth_headers
        )
        patients = res.get_json()["patients"]
        assert len(patients) == 1
        assert patients[0]["email"] == "patient_list_1@test.com"

    def test_list_search_by_first_and_last_name(self, client, auth_headers):
        self._create_patients(client, auth_headers, 3)

        res = client.get("/api/patients?search=First0%20Last0", headers=auth_headers)
        assert res.status_code == 200
        patients = res.get_json()["patients"]
        assert len(patients) == 1
        assert patients[0]["first_name"] == "First0"
        assert patients[0]["last_name"] == "Last0"

        # Order-agnostic
        res2 = client.get("/api/patients?search=Last0%20First0", headers=auth_headers)
        assert res2.status_code == 200
        patients2 = res2.get_json()["patients"]
        assert len(patients2) == 1
        assert patients2[0]["first_name"] == "First0"
        assert patients2[0]["last_name"] == "Last0"
    ...
    def test_list_default_sort_by_last_name(self, client, auth_headers):
        client.post(
            "/api/patients",
            json={"first_name": "Z", "last_name": "Zebra", "dob": "1990-01-01"},
            headers=auth_headers,
        )
        client.post(
            "/api/patients",
            json={"first_name": "A", "last_name": "Apple", "dob": "1990-01-01"},
            headers=auth_headers,
        )
        res = client.get("/api/patients", headers=auth_headers)
        patients = res.get_json()["patients"]
